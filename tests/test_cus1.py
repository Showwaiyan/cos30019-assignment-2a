...
class TestCUS1:
    ...
    def test_finds_path(self):
        cus1 = CUS1(map_0)
        result = cus1.search(origin=1, destinations=[8])
    ...
    def test_different_origin_destinations(self):
        ...
        result = cus1.search(origin=2, destinations=[8])
        assert result.path == [2, 5, 7, 1, 3, 6, 8]
        
    def test_no_solution(self):
        cus1 = CUS1(map_0)
        
        result = cus1.search(origin=8, destinations=[1])
        assert result.path is None
        assert result.destination == [1]
    
    def test_start_equals_goal(self):
        cus1 = CUS1(map_0)
        result = cus1.search(origin=1, destinations=[1])
    ...
    def test_invalid_bounds(self):
        ...
        with pytest.raises(ValueError):
            cus1.search(origin=1, destinations=[99])
