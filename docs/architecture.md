...
classDiagram
    ...
    class Registry {
        ...
        +get_algorithm(name: str) type~TreeSearch~
    }
...
| Decision | Rationale |
|---|---|
| `TreeSearch` is abstract | Enforces uniform `search()` interface across all algorithms via Python `ABC` |
...
| `Parser` is a standalone service | Keeps file I/O concerns out of `TreeSearch` (Single Responsibility Principle) |
